...
async def generate(req: GenerateRequest, authorization: str | None = Header(default=None), x_bridge_token: str | None = Header(default=None)):
    ...
    # Agnes simple mode only supports individual clips. For requests over 20s,
    # automatically use Agnes Creative Video and split the requested duration
    # into scenes, keeping each scene within the per-clip limit.
    if req.duration > 20:
        ...
        data = {
            ...
            "scene_durations_json": scene_durations_json,
            "audio_enabled": "true",
        ...
        # The upstream API accepts only t2v/i2v/ti2vid/keyframes.
        if mode == "cinematic":
            mode = "t2v"
