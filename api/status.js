export default async function handler(req,res){
 ...
 const videoUrl=`${base}/api/video/${encodeURIComponent(id)}`;
 try{
  ...
  const status=d.status||d.state||d.task?.status||d.data?.status||d.result?.status||'processing';
  const normalized=String(status).toLowerCase().replace(/[-_ ]/g,'');
  let done=['completed','success','done','finished','complete','ready','rendered'].includes(normalized);
  const failed=['failed','error','cancelled','canceled'].includes(normalized);
  ...
  if(!done && !failed && Number(progress)>=95){
   try{
    const vr=await fetch(videoUrl,{method:'HEAD',cache:'no-store'});
    if(vr.ok){done=true;progress=100;}
   }catch{}
  }
  if(done)progress=100;
  return res.status(200).json({status,done,failed,progress,videoUrl:done?videoUrl:null,details:d});
