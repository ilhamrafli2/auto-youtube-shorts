export default async function handler(req,res){
  ...
  try{
    ...
    const r=await fetch('https://gateway.pixazo.ai/ltx-video/v1/text-to-video',{
      ...
      body:JSON.stringify({prompt,aspect,num_frames:safeFrames,frame_rate:safeFps,steps:safeSteps})
    });
    const text=await r.text();let data;try{data=JSON.parse(text)}catch{data={raw:text}};
    console.log('[pixazo-video] upstream',{status:r.status,request_id:data?.request_id,model_id:data?.model_id,status_value:data?.status,polling_url:!!data?.polling_url,num_frames:safeFrames,frame_rate:safeFps,steps:safeSteps});
